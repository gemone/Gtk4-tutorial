...
fn main() -> glib::ExitCode {
    ...
    gio::resources_register_include!("tfe_3_r.gresource").expect("Failed to register resources");
...
fn app_open(app: &Application, files: &[gio::File], _hint: &str) {
    let builder = gtk::Builder::from_resource("/com/github/gemone/GTK4-Tutorial/tfe3_r/window.ui");

    let nb = if let Some(nb) = builder.object::<Notebook>("nb") {
        nb
    } else {
        panic!("Could not find object `nb` in window.ui");
    };

    let win = if let Some(win) = builder.object::<Window>("win") {
        win
    } else {
        panic!("Could not find object `win` in window.ui");
    };
